...
public static class PartnerDataService
{
    ...
    public static async Task<Supplier?> GetSupplierAsync(int? supplierID)
    {
        ...
        return await supplierDB.GetAsync(supplierID.Value);
    }
    ...
    public static async Task<int> AddSupplierAsync(Supplier data)
    {
        ...
        var validator = ValidationHelper.Create(data);

        var validationResult = validator
            .Require(s => s.SupplierName, "Tên nhà cung cấp")
            .Require(s => s.ContactName, "Tên giao dịch")
            .Email(s => s.Email, required: false)
            .Phone(s => s.Phone, required: false)
            .Custom(s => !string.IsNullOrWhiteSpace(s.Province) || true, "Tỉnh/thành là thông tin khuyến khích nhập")
            .Validate();
    ...
    public static async Task<bool> UpdateSupplierAsync(Supplier data)
    {
        ...
        if (!ValidationHelper.IsPositiveInteger(data.SupplierID, out var idError, "Mã nhà cung cấp"))
        {
    ...
    public static async Task<Customer?> GetCustomerAsync(int? customerID)
    {
        if (!customerID.HasValue)
            return null;
        return await customerDB.GetAsync(customerID.Value);
    }
    ...
    public static async Task<int> AddCustomerAsync(Customer data)
    {
        ...
        var validationResult = validator
            .Require(c => c.CustomerName, "Tên khách hàng")
            .Require(c => c.ContactName, "Tên giao dịch")
            .Require(c => c.Province, "Tỉnh/Thành")
            .Email(c => c.Email, required: true)
            .Phone(c => c.Phone, required: false)
            .Validate();
        ...
        var isEmailExist = await customerDB.ValidateEmailAsync(data.Email, 0);
        if (isEmailExist)
        {
            throw new ArgumentException($"Email '{data.Email}' đã được sử dụng bởi khách hàng khác");
        }
    ...
    public static async Task<bool> UpdateCustomerAsync(Customer data)
    {
        ...
        if (!ValidationHelper.IsPositiveInteger(data.CustomerID, out var idError, "Mã khách hàng"))
        {
            throw new ArgumentException(idError);
        }
        ...
        // Kiểm tra email trùng lặp
        var isEmailExist = await customerDB.ValidateEmailAsync(data.Email, data.CustomerID);
        if (!isEmailExist)
        {
            throw new ArgumentException($"Email '{data.Email}' đã được sử dụng bởi khách hàng khác");
        }
    ...
    public static async Task<Shipper?> GetShipperAsync(int? shipperID)
    {
        ...
        return await shipperDB.GetAsync(shipperID.Value);
    }
    ...
    public static async Task<int> AddShipperAsync(Shipper data)
    {
        ...
        var validationResult = validator
            .Require(s => s.ShipperName, "Tên người giao hàng")
            .Phone(s => s.Phone, required: false)
            .Validate();
